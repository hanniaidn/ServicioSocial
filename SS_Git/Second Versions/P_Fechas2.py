...
def calcula_ajuste(modelo, estado, date_range, forecast_days):
    # forecast_days is now an int, NOT the widget
...
model_tabs_content = {
    ...
    'Modelo SIR': pn.Row(descripcion_sir, pn.Column(grafica_sir, beta_sir, gamma_sir, t_sir)),
    ...
    'Modelo Bertalanffy-Ivlev': pn.Row(descripcion_bertalanffy, pn.Column(grafica_bertalanffy, L_bi, K_bi, t_exp)),
    'Modelo Janoschek': pn.Row(descripcion_janoschek, pn.Column(grafica_janoschek, beta_jan, L_jan, k_jan, delta_jan, t_exp))
}
...
estadistico_tab = pn.Row(
    pn.layout.HSpacer(width=20),  # Left margin
    pn.Column(
        pn.layout.VSpacer(height=10),  # Top spacing
        control_panel,
        content_area,
        sizing_mode='stretch_width'
    ),
    pn.layout.HSpacer(width=20),  # Right margin
    sizing_mode='stretch_width'
)

# funcio de actualizacion
def update_layout_based_on_file():
    if df_dengue is None:
        ...
        # Resize controls for pre-upload
        ...
        # Resize controls for post-upload
