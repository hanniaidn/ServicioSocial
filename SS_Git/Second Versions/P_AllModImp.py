...
def calcula_ajuste(modelo, estado):
    if modelo == 'Exponencial':
        return ajuste_exponencial(estado)
    elif modelo == 'Logístico':
        return ajuste_logistico(estado)
    elif modelo == 'Richards':
        return ajuste_richards(estado)
    elif modelo == 'Gompertz':
        return ajuste_gompertz(estado)
    elif modelo == 'Bertalanffy-Ivlev':
        return ajuste_bertalanffy(estado)
    elif modelo == 'Janoschek':
        return ajuste_janoschek(estado)
    else:
        return [], [], [], 0
...
@pn.depends(estado_select.param.value, selector_comparativa.param.value)
def grafica_comparativa(estado, modelos_sel):
    if df_dengue is None or estado not in df_dengue.columns:
        return pn.pane.HTML("<b style='color:red;'>Cargue un CSV y seleccione un estado</b>")
    
    fig = go.Figure()
    for modelo in modelos_sel:
        semanas, datos, pred, _ = calcula_ajuste(modelo, estado)
        fig.add_trace(go.Scatter(x=semanas, y=pred, mode='lines', name=f'Ajuste {modelo}'))
    # También mostramos los datos reales
    semanas, datos_reales, _, _ = ajuste_exponencial(estado)
    fig.add_trace(go.Scatter(x=semanas, y=datos_reales, mode='markers+lines', name='Datos Reales'))
